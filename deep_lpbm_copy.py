...
def evaluate_clustering(y_true=None, y_pred=None,
    ...
    if y_true is not None and y_pred is not None:
        # 1. Obtenir l'alignement optimal
        cm_raw, mapping, y_aligned = confusion_matrix_permuted(y_true, y_pred)
        
        #    Cela évite le crash si Q_pred != Q_true ou si des labels sont perdus/fusionnés
        labels = sorted(set(y_true) | set(y_aligned))
        
        # 3. Recalculer la CM avec ces labels explicites pour garantir la correspondance des dimensions
        cm = confusion_matrix(y_true, y_aligned, labels=labels)
        ...
        # Affichage du taux de succès par cluster
        for c in np.unique(y_true):
            mask = y_true == c
            # Attention : on compare y_aligned ici
            correct = np.sum(y_aligned[mask] == c)
            total = np.sum(mask)
            percent = 100 * correct / total if total > 0 else 0
            print(f"Cluster {c}: {correct}/{total} corrects ({percent:.1f}%)")
...
def main(DATA_DIR: str = DATA_DIR, SUBJECT_IDX: int = SUBJECT_IDX, comparison: bool = False, Q_true: int = None):
